...
function [Seizures,Seiz_Totals,timesec,freq,logspec,timedomain,cspectral,id_cspectral,seiz_cepstral,Seiz_score] = swd_detection(eegch,samprate,ratname,day,quality_correction)
...
while done==1  
	...
	if choice==1 | choice==0
		...
		uiwait(msgbox("Choose output folder"));
        outfolder= uigetdir();
		cd(outfolder);
		outputdir=pwd;
        out1 = convertStringsToChars(ratname);
        out2 = convertStringsToChars(day);
		writetable(Seiz_Totals,[out1 '_' out2 '_Seiz_Totals.csv'])
		writetable(Seizures,[out1 '_' out2 '_Seizures.csv'])
	else
	         close
            done=0;
            break
